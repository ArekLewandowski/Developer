...
public class BuildingServiceTest {
	...
	public void testShouldAddAndGetBuilding() {
		...
		// given
		BuildingTO buildingTO = BuildingTO.builder().description("Poznań. Wilda").elevator(true).flatsSum(10).floors(3)
				.localization("ul.Rolna 1").build();
		buildingTO = buildingService.addBuilding(buildingTO);

		BuildingTO buildingTO2 = BuildingTO.builder().description("Poznań. Grunwald").elevator(true).flatsSum(20)
				.floors(6).localization("ul.Kolorowa 1").build();
	...
	public void testShouldUpdateBuilding() {
		...
		// when
		buildingTO.setElevator(false);
		buildingService.updateBuilding(buildingTO);
		BuildingTO gettedBuilding = buildingService.getBuildingById(buildingTO.getId());

		// then
		Assert.assertFalse(gettedBuilding.isElevator());
	}
